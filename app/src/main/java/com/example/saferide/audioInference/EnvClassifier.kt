package com.example.saferide.audioInference
...
/* A classifier that detects if the user is in a potentially dangerous environment for riding */
class EnvClassifier {
...
//        startInferencing()
    ...
    fun inference(): Float {
        ...
        // Return the classification score of vehicle related sounds
        val results = listOf(output[0].categories.find { it.label == "Motor vehicle (road)" }!!.score,
                            output[0].categories.find { it.label == "Car" }!!.score,
                            output[0].categories.find { it.label == "Vehicle horn, car horn, honking" }!!.score,
                            output[0].categories.find { it.label == "Car alarm" }!!.score,
                            output[0].categories.find { it.label == "Skidding" }!!.score,
                            output[0].categories.find { it.label == "Tire squeal" }!!.score,
                            output[0].categories.find { it.label == "Car passing by" }!!.score,
                            output[0].categories.find { it.label == "Truck" }!!.score,
                            output[0].categories.find { it.label == "Air horn, truck horn" }!!.score,
                            output[0].categories.find { it.label == "Bus" }!!.score,
                            output[0].categories.find { it.label == "Emergency vehicle" }!!.score,
                            output[0].categories.find { it.label == "Police car (siren)" }!!.score,
                            output[0].categories.find { it.label == "Ambulance (siren)" }!!.score,
                            output[0].categories.find { it.label == "Fire engine, fire truck (siren)" }!!.score,
                            output[0].categories.find { it.label == "Motorcycle" }!!.score,
                            output[0].categories.find { it.label == "Traffic noise, roadway noise" }!!.score,
                            output[0].categories.find { it.label == "Bicycle" }!!.score,
                            output[0].categories.find { it.label == "Engine" }!!.score,
                            output[0].categories.find { it.label == "Smash, crash" }!!.score)
                            //,  output[0].categories.find { it.label == "Speech"}!!.score)
        val retval = results.maxOrNull()!!
        return retval
    }

    fun startInferencing(interval: Long) {
        startRecording()
        if (task == null) {
            task = Timer().scheduleAtFixedRate(0, interval) {
                val score = inference()
     ...
     * @property    DEFAULT_REFRESH_INTERVAL_MS default refresh interval of the inference
     * @property    LONG_REFRESH_INTERVAL_MS    higher refresh interval for when the accelerometer is running
